...
Taurę papuoškite likusiais mėtos lapeliais.", 3, 50));
            $i++;
...
Su aukščiau nurodytomis proporcijomis galima pagaminti 2-3 kokteilius.", 4, 10));
            $i++;
            array_push($recipes,array('PIENO IR MĖLYNIŲ KOKTEILIS', 7, 'recipeType-5', '', 'recipe-'.$i, 'taste-2', '2014-04-07 18:19:10', 'image-3', "Visus ingredientus sudėkite į kokteilinę arba maišytuvą („blenderį“) ir ~10 sekundžių pamaišykite. Jeigu neturite kokteilinė arba maišytuvo („blenderio“), uogas sudėkite į stiklinę ir pasidarbuokite grūstuvu. Pagal skonį pridėkite cukraus. Gautą turinį supilkite į aukštą arba kokteilinę taurę.", 2, 105));
            $i++;
            array_push($recipes, array('ARBŪZŲ IR ANANASŲ KOKTEILIS', 10, 'recipeType-2', '', 'recipe-'.$i, 'taste-2', '2014-04-02 16:00:05', 'image-4', "Nupjaustykite ananaso ir arbūzo žievę, iš arbūzo pašalinkite sėklas. Sumeskite viską į kokteilinę  arba maišytuvą (blenderį) ir sumeskite ledo kubelius. Maišykite aukštu rėžimu maždaug minutę, išmaišę supilkite į kokteilines taure.", 4, 5));
            $i++;
            array_push($recipes,array('ŠOKOLADINIS BANANINIS KOKTEILIS', 35, 'recipeType-3', '', 'recipe-'.$i, 'taste-1', '2014-04-02 16:00:05', 'image-5', "Visus ingredientus supilkite į plaktuvę su ledo kubeliais, gerai suplakite ir supilkite į aukštą arba kokteilinę taurę.", 5, 29));
            $i++;
...
Pastaba: Jeigu neturite šviežių mėtų, tinka ir džiovintos, tiesa, skonis nebebus toks gaivus.", 4, 1));
            $i++;
            array_push($recipes,array('Sveikatingumo kokteilis', 0, 'recipeType-2', '', 'recipe-'.$i, 'taste-3', '2014-05-02 16:00:05', 'image-11', "Į stikline dėkite ledo gabaliukus ir užpilkite apelsinų sultimis.", 2, 36));
            $i++;
...
Jei sultys rūgščios įmaišome cukraus.", 4, 11));
            $i++;
...
Gaivu ir skanu!", 5, 78));
        ...
        foreach($recipes as $recipeTemp){
            ...
            $recipe->setTotalVote($recipeTemp[10]);
